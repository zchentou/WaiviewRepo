...
from artworks.models import Photo, Topic, Type, Link, CommentLink, ReplyLink, CommentPhoto, ReplyPhoto
...
@register.simple_tag
def link_check_everything(order_type):
    return reverse('link-list', kwargs={'order_type': order_type})


@register.simple_tag
def link_check_topic_type_url(topic_name, type_name, order_type):
    check = reverse('link-topic-type',
                    kwargs={'topic': topic_name, 'type': type_name, 'order_type': order_type})
    check = check.replace('%20', ' ')
    return check


@register.simple_tag
def link_check_type_all_topics_url(type_name, order_type):

    check = reverse(
        'link-type', kwargs={'type': type_name, 'order_type': order_type})
    check = check.replace('%20', ' ')
    return check


@register.simple_tag
def link_check_topic_all_types_url(topic_name, order_type):
    check = reverse(
        'link-topic', kwargs={'topic': topic_name, 'order_type': order_type})
    check = check.replace('%20', ' ')
    return check
...
@register.simple_tag
def order_type_in_url(request):
    if 'date' in request.path:
        return 'date'
    if 'alphabetical' in request.path:
        return 'alphabetical'
...
@register.filter
def all_states_with_data(self):
    will_return = []
    for value in self.values_list('states_choices', flat=True):
        if value and value not in will_return:
            will_return.append(value)
    will_return.sort()
...
@register.filter
def has_some_no_states(self):
    has_some_no_states = False
    for object in self:
        if object.states_choices == '':
            has_some_no_states = True
    return has_some_no_states
